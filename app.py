...
st.markdown("""
    ...
    .stade-badge {
        font-size: 0.8em; text-transform: uppercase; color: #f39c12; letter-spacing: 2px;
    }
    .counter { color: #555; font-size: 0.7em; margin-top: 10px; }
    </style>
...
# Si la pioche n'existe pas ou est vide, on la remplit et on mélange
if 'pioche' not in st.session_state or len(st.session_state.pioche) == 0:
    st.session_state.pioche = list(defis)
    random.shuffle(st.session_state.pioche)

# Si aucun défi n'est affiché (au lancement), on en tire un
...
if st.button("MÉLANGER LES POTIONS"):
    if len(st.session_state.pioche) == 0:
        ...
        st.session_state.pioche = list(defis)
        random.shuffle(st.session_state.pioche)
        st.toast("Le grimoire est plein ! On recommence.", icon="📚")
...
st.markdown(f"""
    <div class="card">
        ...
        <div class="counter">{restant} potions restantes avant mélange</div>
    </div>
    """, unsafe_allow_html=True)
